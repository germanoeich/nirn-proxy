...
### Usage
Binaries coming soon. Docker images can be found [here](https://github.com/germanoeich/nirn-proxy/pkgs/container/nirn-proxy)

Currently there are no ENV vars to configure. Spin up the docker image, point your library to your endpoint instead of discord.com and the proxy will do the rest. The default port is 8080 for the proxy and 9000 for metrics.
...
When using the proxy, it is safe to remove the ratelimiting logic and fire requests instantly to it, however, the proxy does not handle retries. If for some reason (i.e shared ratelimits, internal discord ratelimits, etc) the proxy encounters a 429, it will return that to the client. It is safe to immediately retry requests that return 429 or even setup retry logic elsewhere (like in a load balancer or service mesh).
